...
public class EditCustomerRegion extends TestCase implements IConstants {
  ...
  public void testLoginNG() throws Exception {
	...
	selenium.pdf = new GeneratePDF(data.getpathPDFAdmin(), TEST_CASE_11+"_"+ date + ".pdf");
	selenium.pdf.addTitle("Test Suite : Edit Customer Region");	
	...
	try{ 
	    ...
	    selenium.driver.findElement(By.id("formRegion:regionsTable:regionName:filter")).clear();
	    selenium.driver.findElement(By.id("formRegion:regionsTable:regionName:filter")).sendKeys("AddRegion");
	    Thread.sleep(500);
	    ...
	    selenium.driver.findElement(By.id("formRegionRoles:regionName")).sendKeys("EditRegion");
	    selenium.printScreenshot();
	    selenium.driver.findElement(By.xpath("//button[contains(.,'Save')]")).click();

	    assertEquals("Success to save EditRegion", selenium.driver.findElement(By.xpath("//div[@id='messages_growl_container']/div/div/div[2]/p")).getText());
    	selenium.resultPassed(TEST_CASE_11, date,"admin");
		}catch(Error | Exception e){
			selenium.printScreenshot();
			selenium.resultFailed(e.getMessage().toString(),TEST_CASE_11, date,"admin");
			e.printStackTrace();
		}finally{
		ExecutionResult.reportTestCaseResult(PROJECT, TEST_PLAN, TEST_CASE_11, data.getbuild(), selenium.exceptionNote, selenium.result);
	}
}
