...
public class DeleteLifeCycleLifeCycle extends TestCase implements IConstants {
  ...
  public void testLoginNG() throws Exception {
	...
	selenium.pdf = new GeneratePDF(data.getpathPDFAdmin(), TEST_CASE_16+"_"+ date + ".pdf");
	selenium.pdf.addTitle("Test Suite : Delete Life Cycle");	
	...
	try{ 
	    ...
	    selenium.driver.findElement(By.id("formManageLifeCycle:lifeCycleTableID:attributeValue:filter")).sendKeys("EditLifeCycle");
	    selenium.printScreenshot();
	    ...
	    Thread.sleep(500);
	 	assertEquals("Life cycle has been deleted", selenium.driver.findElement(By.xpath("//div[@id='messages_growl_container']/div/div/div[2]/p")).getText());
	    
	    selenium.printScreenshot();
    	selenium.resultPassed(TEST_CASE_16,date,"admin");
		
		}catch(Error | Exception e){
			selenium.printScreenshot();
			selenium.resultFailed(e.getMessage().toString(),TEST_CASE_16, date,"admin");
			e.printStackTrace();
		}finally{
		ExecutionResult.reportTestCaseResult(PROJECT, TEST_PLAN, TEST_CASE_16, data.getbuild(), selenium.exceptionNote, selenium.result);
	}
}
