...
public class AddCustomerCountry extends TestCase implements IConstants {
  ...
  public void testLoginNG() throws Exception {
	...
	selenium.pdf = new GeneratePDF(data.getpathPDFAdmin(), TEST_CASE_6+"_"+ date + ".pdf");
	selenium.pdf.addTitle("Test Suite : Add Customer Country");	
	...
	try{ 
	    ...
	    selenium.driver.findElement(By.id("formCountryRoles:countryName")).sendKeys("AddCountry");
		selenium.printScreenshot();

	    selenium.driver.findElement(By.xpath("//button[contains(.,'Save')]")).click();
	    
	    Thread.sleep(500);
	    assertEquals("Success to save AddCountry", selenium.driver.findElement(By.xpath("//div[@id='messages_growl_container']/div/div/div[2]/p")).getText());
		selenium.printScreenshot();
    	selenium.resultPassed(TEST_CASE_6, date,"admin");
		}catch(Error | Exception e){
			selenium.printScreenshot();
			selenium.resultFailed(e.getMessage().toString(),TEST_CASE_6, date,"admin");
			e.printStackTrace();
		}finally{
		ExecutionResult.reportTestCaseResult(PROJECT, TEST_PLAN, TEST_CASE_6, data.getbuild(), selenium.exceptionNote, selenium.result);
	}
}
